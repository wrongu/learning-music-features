...
% OLD CODE - USE PAR_ITERATIVE_SPARSE_CODING INSTEAD!!
...
function [A, PHI, phi_norm_hist, mean_err, mean_loss, mean_dense, phi_init] = ...
    iterative_sparse_coding(train_set, n_features, lambda, sigma, max_all_iter, max_inner_iter, A_conv, phi_conv)

    K = length(train_set);
    PHI = cell(n_features, 1); % cell array of basis matrices
    A   = rand(K, n_features); % vectors of weights (one vector per training img), 
                               %    such that the reconstructed image from A(k,:) 
                               %    and PHI is as close to train_set{K} as possible
    
    % initialize PHI to random training imgs
    inds = randperm(K);
    for i=1:n_features
%         PHI{i} = train_set{inds(i)};
        PHI{i} = rand(size(train_set{1})) * 2 - 1;
    end
    
%     plot_imgs_rescale(PHI, 'PHI INIT');
    %pause;
...
%                 pause;
                ...
                arg_to_s = A_k(i)/sigma;
                fprintf('sigma = %d; lambda = %d; arg to s: %g', sigma, lambda, arg_to_s);
...
%             close(h);
        ...
        reconstructed = arrayfun(@(k) img_reconstruct(A(k,:), PHI), 1:K, 'UniformOutput', false);
        max_abs_phi_diff = 0;
        for i = 1:n_features
            weighted_diffs = arrayfun(@(k) A(k,i) * (train_set{k} - reconstructed{k}), 1:K, 'UniformOutput', false);
        ...
        if min_remain > 0
            ...
            fprintf('\tETR: %2d sec :: ETA: very soon\n', sec_remain);
        end
